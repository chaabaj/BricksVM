...
namespace gason
{
		...
		struct EXPORT_DLL JsonNode {
			JsonValue value;
			JsonNode *next;
			char *key;
		};

		struct EXPORT_DLL JsonIterator {
			JsonNode *p;
		...
		inline EXPORT_DLL JsonIterator begin(JsonValue o) {
			return JsonIterator{ o.toNode() };
		}
		inline EXPORT_DLL JsonIterator end(JsonValue) {
			return JsonIterator{ nullptr };
		}

		enum EXPORT_DLL JsonParseStatus {
			JSON_PARSE_OK,
		...
		class EXPORT_DLL JsonAllocator {
			struct Zone {
				Zone *next;
				size_t used;
			} *head = nullptr;
		...
		EXPORT_DLL JsonParseStatus jsonParse(char *str, char **endptr, JsonValue *value, JsonAllocator &allocator);
